    ...
    <article className="max-w-4xl mx-auto px-6 lg:px-10 pt-40 pb-20 space-y-12">
      ...
      <div className="space-y-4">
        {faqs.map((faq, idx) => {
          ...
          return (
            <div key={idx} className="surface-card rounded-2xl border border-[var(--hairline)] overflow-hidden transition-all duration-300">
              <button onClick={() => toggleFaq(idx)} className="w-full px-6 py-5 flex items-center justify-between text-left hover:bg-[var(--surface-2)]/40 transition-colors cursor-pointer">
              ...
              <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isOpen ? "max-h-[300px] border-t border-[var(--hairline)]" : "max-h-0"}`}>
