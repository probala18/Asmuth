...
export default function BestCategoryPage({ params }: { params: Promise<{ category: string }> }) {
  ...
  const catProducts = getProductsByCategory(category.slug).sort((a, b) => b.rating - a.rating);
              ...
              {runners.map((p, idx) => (
                <TiltCard key={p.slug} className="surface-card p-6 rounded-3xl border border-[var(--hairline)] flex flex-col justify-between h-full relative" max={3}>
                  ...
                  <div className="pt-6 border-t border-[var(--hairline)]/50 mt-6 flex items-center justify-between">
                    ...
                    <Link href={`/product/${p.slug}`} className="btn-ghost-glow rounded-full px-4 py-2 text-xs font-semibold">
                      Explore Gear
                    </Link>
                  </div>
                </TiltCard>
