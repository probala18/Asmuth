    ...
    <>
      <section className="relative pt-40 pb-20 px-6 lg:px-10 overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto">
          ...
          <SplitTextReveal text="Collections, deeply edited" className="font-display text-5xl lg:text-7xl font-bold leading-[1.05] tracking-tight" />
          ...
          {collections.map((c) => (
            <TiltCard key={c.title} className="surface-card overflow-hidden group" max={6}>
              <div className="relative aspect-[4/3] overflow-hidden bg-[var(--surface-2)]">
                <img src={typeof c.img === 'string' ? c.img : c.img.src} alt={c.title} loading="lazy" decoding="async" className="size-full object-cover transition-transform duration-700 group-hover:scale-[1.06]" />
              ...
              <div className="p-6">
                ...
                <Link href={`/category/${c.categorySlug}`} className="inline-flex items-center gap-2 text-sm font-semibold text-[var(--emerald-accent)] hover:gap-3 transition-all">
