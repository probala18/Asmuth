...
const Word: React.FC<WordProps> = ({ children, progress, range }) => {
  ...
  return (
    <span className="relative mr-1 mt-[8px] text-2xl font-semibold leading-[0.7] sm:text-3xl">
      <span className="absolute opacity-20 text-[var(--cyan-accent)]">{children}</span>
      <motion.span style={{ opacity }} className="text-[var(--cyan-accent)]">
        {children}
      </motion.span>
    </span>
  );
...
export const MagicText: React.FC<MagicTextProps> = ({ text }) => {
  ...
  return (
    <section className="px-6 py-6 sm:px-8 lg:px-10">
      <div ref={sectionRef} className="mx-auto flex min-h-[220vh] max-w-6xl items-center justify-center py-2">
        <motion.div
          style={{ y, opacity }}
